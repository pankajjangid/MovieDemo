...
import com.demo.themoviedb.interactor.TopRatedIntegrator
...
class TopRatedPresenter(private var view: TopRatedContract.View?) : TopRatedContract.Presenter,
    TopRatedContract.InteractorOutput {


    private val router: Router? by lazy { MyApp.INSTANCE.cicerone.router }
    private var interactor: TopRatedContract.Interactor? = TopRatedIntegrator()
                ...
                when (result) {
                    ...
                    is Result.Success -> {
                        ...
                        val moviesList: List<MoviesResponse.Result> =
                            Gson().fromJson(
                                jokesJsonObject.getJSONArray("results").toString(),
                                type
                            )
